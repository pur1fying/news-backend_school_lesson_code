...
BAASConfig::BAASConfig(const string &path) {
    ...
    if(! BAASUtil::endsWith(split[1], ".json")) throw ValueError("Config file must be a json file : [ " + path + " ]");
...
void BAASConfig::check_config_folder_exist() {
    if(!filesystem::exists(BAAS_CONFIG_DIR)) {
        BAASGlobalLogger->BAASInfo("Make Config Dir : [ " + BAAS_CONFIG_DIR + " ].");
        filesystem::create_directory(BAAS_CONFIG_DIR);
    }
}
